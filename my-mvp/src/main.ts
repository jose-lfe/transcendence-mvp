...
function getHashPage(): Page {
  ...
  if (h !== 'home' && h !== 'versus' && h !== 'tournament') return 'home';
  return h;
}
...
function readPlayers(): string[] {
  ...
  for (let i = 0; i < MAX_TOURNAMENT_PLAYERS; i++) {
    ...
    if (!el) continue;                // <- skip if element not present
  ...
  fill.addEventListener('click', () => {
    ...
    for (let i = 0; i < MAX_TOURNAMENT_PLAYERS; i++) {
      (document.getElementById(`player-${i}`) as HTMLInputElement).value = samples[i] || '';
    }
    updateCount();
  });
  ...
  clear.addEventListener('click', () => {
    for (let i = 0; i < MAX_TOURNAMENT_PLAYERS; i++) {
      (document.getElementById(`player-${i}`) as HTMLInputElement).value = '';
    }
    updateCount();
  });
